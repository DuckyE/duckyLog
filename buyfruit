...
local function sendPurchaseCard(id, price)
    ...
    local embed = {
        author = { name = player },
        title  = "🛍️ Fruit Purchased!",
        color  = 0x2ECC71,
        fields = {
            { name = "👤 Player", value = "`"..player.."`", inline = true },
            { name = "🍎 Fruit",  value = "`"..fruitIcon(id).." "..labelOf(id).."`", inline = true },
    ...
    while RUNNING do
        ...
        for _,item in ipairs(wanted) do
            ...
            while bought<MAX_PER_FRUIT_PER_TICK and budgetLeft>=item.price and netWorth>=item.price do
                ...
                if not ok then
                    if VERBOSE then notify("❌ ซื้อไม่สำเร็จ",labelOf(item.id).." ("..tostring(err)..")",3) end
                    break
                end
                bought=bought+1
                ...
                if VERBOSE and PURCHASE_TOAST then
                    notify("✅ ซื้อสำเร็จ",labelOf(item.id).." ฿"..comma(item.price),2)
                end
